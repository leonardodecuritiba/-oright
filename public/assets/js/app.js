...
+function($){
  ...
  provider.list = {
    ...
    validation: {
      ...
      css:      '',
      js:       [
          'bootstrap-validator/validator-bs4.min.js'
          ],
    },
...
+function($){
  ...
  provider.initDropify = function() {
    ...
    provider.provide('dropify', function(){
      $(this).dropify({
          messages: {
              'default': 'Arraste ou busque um arquivo local',
              'replace': 'Arraste ou busque um arquivo local para substituir',
              'remove':  'Remover',
              'error': {
                  'fileSize': 'O arquivo é maior que ({{ value }} max).',
                  'minWidth': 'A largura da imagem é menor que ({{ value }}}px min).',
                  'maxWidth': 'A largura da imagem é maior que ({{ value }}}px max).',
                  'minHeight': 'A altura da imagem é menor que ({{ value }}}px min).',
                  'maxHeight': 'A altura da imagem é maior que ({{ value }}px max).',
                  'imageFormat': 'Esse formato não é aceito. apenas ({{ value }} ).'
              }
          }
      });
    });
