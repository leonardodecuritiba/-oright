...
class DataHelper {
    ...
    static public function setMoipDate($value)
    {
        if ($value == NULL) return $value;
        return self::getMoipDate($value)->toDateTimeString();
    }

    static public function getMoipDate($value)
    {
        if ($value == NULL) return $value;
        $dt = Date::now();
        return $dt->year($value->year)->month($value->month)->day($value->day)
